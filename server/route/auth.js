...
router.post('/register', async (req, res) => {
  try {
    ...
    if (!dob.day) {
      return res.status(400).json({ error: 'Date of birth day is required' });
    }
    if (!dob.month) {
      return res.status(400).json({ error: 'Date of birth month is required' });
    }
    if (!dob.year) {
      return res.status(400).json({ error: 'Date of birth year is required' });
    }
